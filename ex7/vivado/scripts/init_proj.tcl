...
# Create new project.
create_project $project_name . -part xc7z020clg400-1 -force
set_property board_part digilentinc.com:zybo-z7-20:part0:1.0 [current_project]
set_property target_language Verilog [current_project]

# Add source and constraint files.
add_files [glob ../sourcecode/*.sv]
add_files [glob ../sourcecode/*wrap.v]
add_files [glob ../sourcecode_hls/*.v]
add_files -fileset constrs_1 -norecurse ./constraints/zybo-z7.xdc
update_compile_order -fileset sources_1

# Define macro `XILINX` with value `1`.
...
proc gen_dc_fifo {scope} {
    ...
    startgroup
        ...
        if {$scope eq "hdmi_rx"} {
            set_property -dict [list \
                    CONFIG.Read_Clock_Frequency     {170} \
            ...
            set_property -dict [list \
                    ...
                    CONFIG.Write_Clock_Frequency    {170} \
            ] [get_bd_cells $path]
        }
    endgroup
...
connect_bd_net [get_bd_pins hdmi_rx/rst_gen/clk_i]  [get_bd_pins hdmi_rx/clk_gen/clk_out1]
...
connect_bd_net [get_bd_pins hdmi_rx/dc_fifo/rd_clk] [get_bd_pins hdmi_rx/clk_gen/clk_out1]
create_bd_cell -type ip -vlnv xilinx.com:ip:xlconstant:1.1 hdmi_rx/const1
connect_bd_net [get_bd_pins hdmi_rx/dc_fifo/wr_en]  [get_bd_pins hdmi_rx/const1/dout]
...
# Outputs
connect_bd_net [create_bd_pin -dir O hdmi_rx/proc_clk_o]    [get_bd_pins hdmi_rx/clk_gen/clk_out1]
...
create_bd_cell -type ip -vlnv xilinx.com:ip:xlslice:1.0 slice_switch_20
set_property -dict [list \
        CONFIG.DIN_WIDTH    4 \
        CONFIG.DIN_FROM     2 \
        CONFIG.DIN_TO       0 \
        CONFIG.DOUT_WIDTH   3 \
    ] [get_bd_cells slice_switch_20]
connect_bd_net [get_bd_ports switch_i] [get_bd_pins slice_switch_20/Din]
connect_bd_net [get_bd_pins slice_switch_20/Dout] [get_bd_pins rgb_proc/switch_i]
...
create_bd_cell -type ip -vlnv xilinx.com:ip:xlconstant:1.1 hdmi_tx/const1
connect_bd_net [get_bd_pins hdmi_tx/dc_fifo/rd_en]  [get_bd_pins hdmi_tx/const1/dout]
...
#additional IOs
startgroup
create_bd_port -dir I -from 2 -to 0 btn_i
connect_bd_net [get_bd_pins /rgb_proc/btn_i] [get_bd_ports btn_i]
endgroup

startgroup
create_bd_port -dir O -from 3 -to 0 led_o
connect_bd_net [get_bd_pins /rgb_proc/led_o] [get_bd_ports led_o]
endgroup

#synth implment stragtegy
...
add_files -fileset sim_1 -norecurse -scan_for_includes [glob ../sourcecode/tb/*.sv]
...
# Disable OOC synthesis to enable retiming.
